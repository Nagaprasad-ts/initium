...
class RegistrationController extends Controller
{
        ...
        return DB::transaction(function () use ($validated, $event) {
            ...
            try {
                ...
                $orderData = [
                    ...
                    'amount' => $registration->total_amount * 100, // in paise
                    'currency' => 'INR',
                    'notes' => [
                ...
                return response()->json([
                    ...
                    'contact_name' => $validated['registration_type'] === 'individual' ? $validated['name'] : ($validated['team_name'] . ' Leader'),
                    'contact_email' => $validated['contact_email'],
                    'contact_phone' => $validated['contact_phone'],
                    'event_name' => $event->name,
                ]);
            } catch (\Exception $e) {
                // For development/demo if keys are not set
                if (config('app.debug')) {
                    return response()->json([
                    ...
                    ], 200); // 200 to allow frontend to continue in mock mode
                }
                throw $e;
            }
        });
    ...
    /**
     * Display the success page for registration.
     */
